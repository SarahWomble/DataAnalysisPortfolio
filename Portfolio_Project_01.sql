...
SELECT 
  ROUND(AVG(Winner_Pts - Loser_Pts), 0) AS Points_won_by
FROM 
  superbowl s ;
...
SELECT 
  COUNT(SB) AS NumberOfSuperBowls 
FROM superbowl s ;
...
SELECT 
  ROUND(AVG(Winner_pts), 0) AS average_winning_score
FROM 
  superbowl s ;
...
SELECT 
  MAX(Loser_Pts) AS Highest_losing_score 
FROM 
  superbowl s ;
-- The highest losing score is 33 points
SELECT 
  MIN(Winner_Pts) AS Lowest_winning_score 
FROM 
  superbowl s ;
...
WHERE 
  Loser_Pts = 33;
-- Super bowl 52: The New England Patriots lost to the Philadelphia Eagles
-- The score was 41-33
...
WHERE 
  Winner_Pts = 13;
-- Super bowl LIII (53): The New England Patriots beat the Los Angeles Rams with a score of 13-3
...
FROM 
  superbowl s 
WHERE 
  (Winner = 'New England Patriots' AND Loser  = 'Pittsburgh Steelers')
  OR (Winner = 'Pittsburgh Steelers' AND Loser = 'New England Patriots')
ORDER BY 
  Winner;
-- There is no data from this query, therefore they have never played each other in a super bowl

-- What is the most common Stadium, City, and State where super bowls are played?
SELECT 
  Stadium, 
  COUNT(Stadium) AS Number_of_SBs_Played 
FROM 
  superbowl s
GROUP BY 
  Stadium
ORDER BY 
  Number_of_SBs_Played DESC;
-- The Orange Bowl, Rose Bowl, and Louisiana Superdome have all hosted the most (5) super bowls

SELECT 
  City, 
  COUNT(City) AS Number_of_SBs_Played 
FROM 
  superbowl s
GROUP BY 
  City
ORDER BY 
  Number_of_SBs_Played DESC;
-- New Orleans has hosted the most super bowl games (10)
-- The second most played city being Miami Gardens (6)

SELECT 
  State, 
  COUNT(State) AS Number_of_SBs_Played 
FROM 
  superbowl s
GROUP BY 
  State
